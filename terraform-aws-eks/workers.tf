...
resource "aws_autoscaling_group" "workers" {
  ...
  vpc_zone_identifier     = ["${split(",", coalesce(lookup(var.worker_groups[count.index], "private_subnets", ""), local.workers_group_defaults["private_subnets"]))}"]
...
resource "aws_launch_configuration" "workers" {
  count                       = "${var.worker_group_count}"
  name_prefix                 = "${aws_eks_cluster.this.name}-${lookup(var.worker_groups[count.index], "name", count.index)}-lc"
