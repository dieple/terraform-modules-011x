locals {
  ...
  workers_group_defaults_defaults = {
    ...
    private_subnets               = "${join(",", var.private_subnets)}" # A comma delimited string of subnets to place the worker nodes in. i.e. subnet-123,subnet-456,subnet-789
    autoscaling_enabled           = "${var.autoscaling_enabled}"        # Sets whether policy and matching tags will be added to allow autoscaling.
    ...
    iam_instance_profile_name     = "${var.iam_instance_profile_name}"  # A custom IAM instance profile name. Used when manage_worker_iam_resources is set to false. Incompatible with iam_role_id.
