...
resource "aws_route_table" "public" {
  ...
  tags = "${merge(var.tags, var.public_route_table_tags, map("Name", format("%s-public-%s", var.name, substr(element(var.azs, count.index),-1,1))))}"
}
...
resource "aws_route_table" "private" {
  ...
  tags = "${merge(var.tags, var.private_route_table_tags, map("Name", (var.single_nat_gateway ? "${var.name}-private" : format("%s-private-%s", var.name, substr(element(var.azs, count.index),-1,1)))))}"
...
resource "aws_route_table" "database" {
  ...
  tags = "${merge(var.tags, var.private_route_table_tags, map("Name", (var.single_nat_gateway ? "${var.name}-database" : format("%s-db-%s", var.name, substr(element(var.azs, count.index),-1,1)))))}"
}
...
resource "aws_subnet" "public" {
  ...
  tags = "${merge(var.tags, var.public_subnet_tags, map("Name", format("%s-public-%s", var.name, substr(element(var.azs, count.index),-1,1))))}"
}
...
resource "aws_subnet" "private" {
  ...
  tags = "${merge(var.tags, var.private_subnet_tags, map("Name", format("%s-private-%s", var.name, substr(element(var.azs, count.index),-1,1))))}"
}
...
resource "aws_subnet" "database" {
  ...
  tags = "${merge(var.tags, var.database_subnet_tags, map("Name", format("%s-db-%s", var.name, substr(element(var.azs, count.index),-1,1))))}"
}
...
resource "aws_subnet" "redshift" {
  ...
  tags = "${merge(var.tags, var.redshift_subnet_tags, map("Name", format("%s-redshift-%s", var.name, substr(element(var.azs, count.index),-1,1))))}"
}
...
resource "aws_subnet" "elasticache" {
  ...
  tags = "${merge(var.tags, var.elasticache_subnet_tags, map("Name", format("%s-elasticache-%s", var.name, substr(element(var.azs, count.index),-1,1))))}"
}
...
resource "aws_eip" "nat" {
  ...
  tags = "${merge(var.tags, map("Name", format("%s-eip-%s", var.name, substr(element(var.azs, (var.single_nat_gateway ? 0 : count.index)),-1,1))))}"
}
